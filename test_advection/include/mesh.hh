...
struct mesh_t : vt::Collection<mesh_t, vt::Index1D> {
    ...
    void init_() { 
        ...
        x.assign(npoints_, 0.) ;
        u.assign(npoints_, 0.) ;
        u_old.assign(npoints_, 0.) ;
        a.assign(npoints_, 1.) ; // for now a = 1  constant 
        rhs.assign(npoints_, 0.) ;
    ...
    void calc_rhs() {
        ...
        loop_interior(
            [&] (const std::size_t & idx) {
                rhs[idx] =  inv_dx*(flux[idx] - flux[idx+1]); 
            }
        ) ; 
